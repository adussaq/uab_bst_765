    ...
    self.onmessage = function (event) {
        ...
        for (i = 0; i < length; i += 1) {
            if (Math.random() < event.data.uniform) {
                sam = globalObject.norm.inv(Math.random(), event.data.mean1, event.data.stdev1);
            } else {
                sam = globalObject.norm.inv(Math.random(), event.data.mean2, event.data.stdev2);
            }            
            total += sam;
